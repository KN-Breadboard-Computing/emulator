...
void print_emulator_status(Emulator *emulator) {
    ...
    mvprintw(y++,x,"flag: S P Z C O - - -");x+=4;
        ...
        for (int i = 0; i < memory_tables_count; ++i) {
            switch (current_memory_view_mode[i]) {
                ...
                case DEC_SIGNED:
                    ...
                    break;
                case INST:
                    mvprintw(y,x+=spacing[current_memory_view_mode[i]],"X"); x+=5;
                    break;
                case ASCII:
                    for (int j=0;j<16;j++){
                        if (emulator->memory[j+16*k]>32)
                            mvprintw(y,x+=spacing[current_memory_view_mode[i]],"%c",emulator->memory[j+16*k]);
                        else mvaddch(y,x+=spacing[current_memory_view_mode[i]],'.');
                    }
                    break;
            } x+=spacing_between;
